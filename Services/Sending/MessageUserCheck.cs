...
{
    public class MessageUserCheck : Message
    {
        ...
        public override byte[] GetByte()
        {
            ...
            headerData.Append(Util.SpacePadding("USER_CHECK", 100)); // Key 100자리 공백 패딩
            headerData.Append(Util.ZeroPadding(0, 6)); // SubKey 6자리 0 패딩

            // Body Data 생성
            StringBuilder bodyData = new StringBuilder();
            bodyData.Append(Util.SpacePadding(UserId, 40)); // 사용자 ID
            bodyData.Append(Util.SpacePadding(UserPwd, 100)); // 사용자 PWD
